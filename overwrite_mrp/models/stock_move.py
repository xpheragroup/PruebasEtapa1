...
class Override_StockMove(models.Model):
    ...
    deviation_per = fields.Float(string='Desviación Porcentual', compute='_compute_custom_values', digits=(12,4))
    ...
    @api.depends('std_quantity', 'product_qty', 'product_id.standard_price')
    def _compute_custom_values(self):
        for record in self:
            record.deviation = record.product_qty - record.std_quantity
            record.deviation_per = (record.deviation * 100) / record.std_quantity if record.std_quantity > 0 else 100
