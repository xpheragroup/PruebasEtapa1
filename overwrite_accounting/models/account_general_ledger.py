...
class AccountGeneralLedgerReport(models.AbstractModel):
    ...
    filter_hierarchy = True                                                             # Overdefinition to remember class member

    @api.model              
    def _get_columns_name(self, options):                                               # Overdefinition to add partner VAT column (NIT in Colombia)
        return [
            {'name': ''},
            {'name': _('Date'), 'class': 'date'},
            {'name': _('Communication')},
            {'name': _('NIT')},                                                         # Added partner VAT here!                     
            {'name': _('Partner')},
    ...
    @api.model  
    def _get_aml_line(self, options, account, aml, cumulated_balance):                  # Overdefinition to add partner VAT column (NIT in Colombia)
        if aml['payment_id']:
        ...
        return {
            ...
            'columns': [
                ...
                {'name': aml['partner_vat'], 'title': aml['partner_vat'], 'class': 'whitespace_print'}, #Added here!
    ...
    @api.model
    def _get_query_amls(self, options, expanded_account, offset=None, limit=None):      # Overdefinition to bring parner.vat
        ...
        _, where_clause, where_params = self._query_get(new_options, domain=domain)
